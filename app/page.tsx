        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="grid md:grid-cols-4 gap-6">
            ...
            {productsData.slice(0, 4).map((item) => (
              <a href={`/${item.category}/${item.id}`} key={item.id} className="block p-6 bg-slate-50 rounded-xl border border-slate-100 hover:border-secondary/20 hover:shadow-lg transition-all cursor-pointer group">
                <h3 className="text-lg font-bold text-primary mb-2 group-hover:text-secondary transition-colors">{item.name}</h3>
