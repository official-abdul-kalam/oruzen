        ...
        >
            <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
                ...
                <div className="hidden md:flex items-center gap-8">
                        ...
                        <div className="absolute top-full left-1/2 -translate-x-1/2 pt-4 opacity-0 translate-y-2 pointer-events-none group-hover:opacity-100 group-hover:translate-y-0 group-hover:pointer-events-auto transition-all duration-300 w-[650px] z-50">
                            <div className="bg-white rounded-3xl shadow-2xl shadow-primary/10 border border-slate-200 overflow-hidden flex">
                                ...
                                <div className="w-1/3 bg-slate-50 p-4 border-r border-slate-100 flex flex-col gap-2">
                                    ...
                                    <Link href="/apps" onMouseEnter={() => setActiveCategory('apps')} className={`px-4 py-3 text-sm font-bold rounded-xl transition-all flex items-center gap-3 ${activeCategory === 'apps' ? 'bg-white border text-primary border-primary/20 shadow-sm' : 'border border-transparent text-slate-600 hover:bg-slate-100'}`}>
                                        <Smartphone size={18} className={activeCategory === 'apps' ? 'text-primary' : 'text-slate-400'} /> Apps
                                    </Link>
                                    <Link href="/open-source" onMouseEnter={() => setActiveCategory('open-source')} className={`px-4 py-3 text-sm font-bold rounded-xl transition-all flex items-center gap-3 ${activeCategory === 'open-source' ? 'bg-white border text-primary border-primary/20 shadow-sm' : 'border border-transparent text-slate-600 hover:bg-slate-100'}`}>
                                        <Terminal size={18} className={activeCategory === 'open-source' ? 'text-primary' : 'text-slate-400'} /> Open S.
                                    </Link>
                                    <div className="h-px bg-slate-200 my-1"></div>
                                    <Link href="/projects" onMouseEnter={() => setActiveCategory('projects')} className={`px-4 py-3 text-sm font-bold rounded-xl transition-all flex items-center gap-3 ${activeCategory === 'projects' ? 'bg-white border text-primary border-primary/20 shadow-sm' : 'border border-transparent text-slate-600 hover:bg-slate-100'}`}>
                                        <Code2 size={18} className={activeCategory === 'projects' ? 'text-primary' : 'text-slate-400'} /> Student Projects
                                    </Link>
                                </div>
                    ...
                    {authLoading ? (
                        <div className={`w-[80px] h-[42px] rounded-xl animate-pulse ${theme === 'dark' && !scrolled ? 'bg-white/20' : 'bg-slate-200'}`}></div>
                    ) : user ? (
